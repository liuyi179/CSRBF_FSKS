 % alldata做中心点  计算误差的时候评估点用的alldata，画图的时候评估点用的 eg1_epoints
% 区域[-1,1]
clear all
tic
%Wendland C6 RBF函数
...
for j=1:K
    ...
    DM = DistanceMatrixCSRBF1(epoints,xctrs{j},ep(j));
    EM{j} = rbf(ep(j),DM);    
    %% 内部基函数矩阵块
    a = DistanceMatrixCSRBF1(intdata,xctrs{j},ep(j));
    aa = DistanceMatrixCSRBF1(alldata,xctrs{j},ep(j));
    A{j}=rbf(ep(j),a);    %A
    AA{j}=rbf(ep(j),aa);
    ...
    %% 边界基函数矩阵块
    bdy = DistanceMatrixCSRBF1(bdydata,xctrs{j},ep(j));
    BDY{j} = rbf(ep(j),bdy);    
    %% 求矩阵块儿
    if(j==1)
        nEM=EM{1};
...
pfTi = nEM*v;
pfTr = nEM*w;
exactTe = Te(epoints(:,1),epoints(:,2),epoints(:,3),k*dt);  
exactTi = Ti(epoints(:,1),epoints(:,2),epoints(:,3),k*dt);
exactTr = Tr(epoints(:,1),epoints(:,2),epoints(:,3),k*dt);
maxerrTe = norm(pfTe-exactTe,inf); 
maxerrTi = norm(pfTi-exactTi,inf);
maxerrTr = norm(pfTr-exactTr,inf);
rms_errTe = norm(pfTe-exactTe)/sqrtm(length(epoints));  
rms_errTi = norm(pfTi-exactTi)/sqrtm(length(epoints));  
rms_errTr = norm(pfTr-exactTr)/sqrtm(length(epoints)); 
fprintf('N:  %d\ngridtype:  h\n', length(alldata))
fprintf('RMS error Te:   %e\n', rms_errTe)
fprintf('RMS error Ti:   %e\n', rms_errTi)
fprintf('RMS error Tr:   %e\n', rms_errTr)
fprintf('Maximum error Te:   %e\n', maxerrTe)
fprintf('Maximum error Ti:   %e\n', maxerrTi)
fprintf('Maximum error Tr:   %e\n', maxerrTr)